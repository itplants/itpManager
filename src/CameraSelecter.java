...
//		System.out.println("System cam size="+ITPlanterClass.getSystemCameraSize());
	    ...
	    System.out.println("CameraSellecrerClass: Files.getCamNo() "+Files.getCamNo());
	...
	class setCamNum implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {	//設定ボタンが押されると、このメソッドが呼ばれる		
			...
			for(int j=0;j<ITPlanterClass.getSystemPlanterSize();j++){// All planters
				...
				p.getInformation().removeAllCameraName();
...
//System.out.println("CameraSelecter: setCameraName1　　i="+i+" "+cname);
				p.getInformation().addCameraName(cname);
				cname = p.getInformation().getCameraName(i);
...
/*			
					for(int k=0;k<popupCam.getComponentCount();k++){
					menu = (JMenuItem) popupCam.getComponent(k);
					String s1=menu.getText();// selec		//
					for(int no=0;no<ITPlanterClass.getPlanterList().size();no++){
						PlanterClass p1 = ITPlanterClass.getPlanterList().get(no);					
						if(s.contains(checkMark)){
							s=s.replace(checkMark, "");
							System.out.println("CameraSelecterClass: "+no+"が抹消されました。|"+s.replace(checkMark, "")+"|");
						}
						else{
							s=checkMark+s;
							System.out.println("CameraSelecterClass: "+no+"が登録されました。|"+s.replace(checkMark, "")+"|");
						}
						menu.setText(s);					
						//					    			
						break;
