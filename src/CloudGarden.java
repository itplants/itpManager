...
//	while( itpCom1.messageUse==false ); // wait
	...
	rmessage += "Temperature: " +ITPlanterClass.getCurrentPlanterClass().getSensor().getSensorTemprarure()+" \\% " +ITPlanterClass.getCurrentPlanterClass().getSensor().getTemperatureWarningLevel()+"\r\n";// get_TempWarnFlag	
	rmessage += "WaterLevel: "  +ITPlanterClass.getCurrentPlanterClass().getSensor().getSensorWaterlevel()+" \\% "+ITPlanterClass.getCurrentPlanterClass().getSensor().getWaterWarningLevel()+"\r\n";// get_WaterWarn	
	rmessage += "Illumination: "+ITPlanterClass.getCurrentPlanterClass().getSensor().getSensorIllumination()+" "  +ITPlanterClass.getCurrentPlanterClass().getSensor().getIlluminationWarningLevel()+"\r\n"; // get_illumWarn	
	rmessage += "\r\n";
...
public   void Login_processing() 
{
    ...
    {// Login_processing
	String data=Files.getSetupfile();
	String[] arg=null;
	...
	//メッセージの送信処理
		...
		if(arr.length!=3){
			// Error
			itp_Logger.logger.info("Cloud-Garden Serverが応答しません。");
			return;
		}
		arr=arr[2].split("\r\n",0);
		if (arr.length < 1) {
...
//
// main loop
public void itplanter_run(String email,String passwd,String planter_name)
{
...
private  void PLT_processing() 
{
	...
	for(int i=0;i<pd.length;i++){//
	String[] pp= pd[i].split(" ");
		if(pp[0].compareTo("Planter")==0){
			if(pp.length>=2){
			pn++;
			if(pn>get_nPlanter()) continue;
			message += ("Name: "+pp[1]+"\r\n");
			setPlanterName(pp[1]); // set Planter Name
			message += ("Number: "+(k++)+" ");// Planter Number
			setPlanterNumber(k-1);
			} 
		} else
		if(pp[0].compareTo("Version")==0){
			if(pp.length>=2){
				message +=  ( "/ Version: "+pp[1]+" ");
				setVersion(pp[1]);// set version
				}
			} else
		if(pp[0].compareTo("Serial")==0){ 
			if(pp.length>=2){
					message += (" / Serial: "+pp[1]+" ");
					setSerial(Integer.parseInt(pp[1].replace("[^0-9]", "")));
				}
		} else
		if(pp[0].compareTo("Camera")==0){ 
			if(pp.length>=2){
				if(pp[1].compareTo("yes")==0 || pp[1].compareTo("Yes")==0 || pp[1].compareTo("YES")==0){
					message += ("/ Camera: 1\r\n");
					setCamera(true);
				}
			else{
	...
	if(er[0].compareTo("205")==0){
		...
		if(c[0].compareTo("Amount:")!=0){
			JOptionPane.showMessageDialog(null, "Server Error：期待したエラーメッセージではない");
...
private void RSI_processing(String[] arr) {
	...
	s="";
	s += "Temp " + ITPlanterClass.getCurrentPlanterClass().getSensor().getSensorTemprarure()+"\r\n";
	s += "Water "+ ITPlanterClass.getCurrentPlanterClass().getSensor().getSensorWaterlevel()+"\r\n";
	s += "illum "+ ITPlanterClass.getCurrentPlanterClass().getSensor().getSensorIllumination()+"\r\n";
	}
	itpCom1.say("300 OK\r\n"+s+"\r\n");
	itp_Logger.logger.info("RSI:"+System.getProperty("line.separator")+s);
}
...
//	for(int i=1;i<=get_nPlanter();i++){
		...
		if(IsMacorWin.isMacOrWin()==true ){
			// Mac
	       	// full path need.
        	File f = new File("tif.txt");
        	try 
        	{	
        		FileInputStream fi = new FileInputStream(f);
        		fi.close();
    			pname=pname.replace( ".jpg",".tif");
    			itp_Logger.logger.info("TIF :"+pname);
        	}
        	catch(Exception e)
        	{
    			//
		...
		itp_Logger.logger.info("FileName["+i+"]="+pname);
...
public void CMD_processing(String[] arr) 
{
	...
	// CMD
		...
		if( arr.length >= 2 ){
			...
			if(commandline.length>1) Scombuf = commandline[1];
		}
		
		// Arg1: 100　etc.
		if( arr.length >= 3 ){
			argline=arr[2].split(" ",0);
			if(argline.length>1){
				if(commandline.equals("H")){
					Scombuf += (5*Integer.parseInt(argline[1]));	// command line		
				} else {
					Scombuf += argline[1];	// command line							
				}
			}
		}
		// Arg2以上の引数は使わないので無視する。
...
//			String rmessage="illum "+planterStatus.get(currentPlanterNo-1).get_ADillum();
		...
		if(commandline[1].indexOf("%")==0){
			/// to do search serial No.
			currentPlanterNo=Integer.parseInt(argline[1].replaceAll("[^0-9]",""));
			if(currentPlanterNo <=0 )currentPlanterNo=1;// default planter No.
		...
		itp_Logger.logger.info("Command|: "+String.valueOf(Scom)+"|");// Debug
		...
		///
				...
				arr=result.split(System.getProperty("line.separator"),0);
				// SendComが受けたコマンドが正しいかどうかの検証
				if( arr[0].compareTo( String.valueOf("Command: "+Scombuf)) == 0){
					itpCom1.say("300 OK\r\n");					
					itpCom1.say(arr[1]+"\r\n\r\n"); // status report
				} else {
					itpCom1.say("400 Error\r\n");
...
private String toASCII(String result) {
	//
	//search  "Command: T\n"
	String s=result.substring(0, 10);
	String[] p=s.split("Command: ");
	if(p[1].compareTo("T")!=0) return null;
	String d=result.substring(11, result.length());
	for(int i=0;i<result.length();i++){
		String a=d.substring(1,i+1);
		s += String.format("\"%d\"", Integer.valueOf(a));
	}
		s+="\r\n\r\n";

		itp_Logger.logger.info("T="+s);
	return null;
}

//
public void serial_set(String pnames) 
{
	//planterName=new String[nPlanter];
	
	for(int i=0;i<get_nPlanter();i++){
	//int	i=currentPlanterNo;
	String[] arr=null;
	String s="";
		...
		if(ps.length<1) return;
			...
			if(planterVersion.get(i)==2){// itpOS ver.2
				...
				String[] ps=arr[1].split(" ",0);
				if(ps.length<1) return;
					planterSerial.add(Integer.valueOf(ps[0]));
			}// itpOS ver.2
...
//	Planter Name reader
	...
	///
	itplanter_run(arg[0],arg[1], pname);
}
