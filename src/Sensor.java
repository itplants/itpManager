...
public class Sensor {
	...
	public static double waterlevel=0.0;
	public static double illumination=0.0;
...
//
	...
	private double illumCorrect=1.0;
	
	public void setTotalCamNo(int n)
	{
		totalCamNo=n;
	}
	public boolean getTemperatureReport()
	{
		return temperatureReport;
	}
	
	public boolean getWaterlevelReport()
	{
		return waterlevelReport;
	}
	
	public boolean getIlluminationReport()
	{
	...
	public void setWaterLevelWarningLevel(double w)
	{
		...
		setValue(Command);
		itp_Logger.logger.info("setWaterLevelWarningLevel "+String.valueOf(temperature));
	}
...
//		return (double)(AD/1024*3.3*1000-400)/19.5;
	...
	public String getSensors()
	{
		...
		String t="";
		String u="";
		int sysPlantSize=ITPlanterClass.getSystemPlanterSize();
System.out.println("SensorClass: getSystemPlanterSize:"+sysPlantSize);
...
// DUMMY-PLANTER
	...
	{
			String Command="A";
			temperature=getDoubleValue(Command);
//			System.out.println("version "+ITPlanterClass.getCurrentPlanterClass().getVersion().getMejar());
			...
			if(Version.getMejar()<2){
				// ITPLANTER-01の値　AD値
				temperature=ADToTemp((int)temperature);
			}
...
//			
			if(temperature > getTemperatureWarningLevel()){
				temperatureReport=true;
				sendMail.addToText(String.valueOf(temperature));
			}
//
			return temperature+tempCorrect;
	}
...
//			System.out.println("retrun:|"+wr+"|");
			String[] arg=wr.split(System.getProperty("line.separator"));
			waterlevel=0.0;
			//
			ITPlanterClass.getCurrentPlanterClass().getVersion();
			if(Version.getMejar()<2){
	...
	// FIle save & load
		private static String setupFile()
		{
			...
			if(IsMacorWin.isMacOrWin()==true ){
				// MacOSX
				SETUPFILE=sCurrentDir+"/data/Mac/"+ITPlanterClass.getCurrentPlanterClass().getInformation().getPlanterName()+"_Comp.txt";// MACOSX  
			} else {
				// Windows
				SETUPFILE=sCurrentDir+"\\data\\Win\\"+ITPlanterClass.getCurrentPlanterClass().getInformation().getPlanterName()+"_Comp.txt";// WIndows
			}
			return SETUPFILE;
		}
		...
		public   String fread(){
			...
			{
				//JOptionPane.showMessageDialog(null,"設定ファイルが見つかりません。itpManagerをインストールしなおしてください。");
				//e.printStackTrace();
