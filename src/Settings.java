...
public class Settings extends JPanel implements ActionListener, MouseListener {
	...
	private JCheckBox getCheckSaveHolder() {
		if (checkSaveHolder == null) {
			...
			checkSaveHolder.setSelected(true);
			} else 
...
//			String[] combodata = {"通常栽培", "夜間栽培", "節電栽培", "ピークシフト"};
	...
	private JTextField getWhiteLEDSpeed() {
		if (whiteLEDSpeed == null) {
			...
			String result=sendCom.sendcom(" "+(ITPlanterClass.getCurrentPlanterNo()+1)+" -e c");
			String[] s=result.split(System.getProperty("line.separator"));
			if(s.length>1){
			String[] t=s[1].split(" ");
			if(t[1].equals(":")==false){
	...
	private JTextField getPumpBuustTime() {
		if (pumpBoostTime == null) {
			...
			// プランターがないとエラーになるので
			if(ITPlanterClass.getSystemPlanterSize()>0){
			String result=sendCom.sendcom(" "+(ITPlanterClass.getCurrentPlanterNo()+1)+" -e j");
			String[] s=result.split(System.getProperty("line.separator"));
			if(s.length>1){
			String[] t=s[1].split(" ");
			if(t[1].equals(":")==false){
	...
	private JTextField getPC2AutoTime() {
		if (pC2AutoTime == null) {
			...
			// プランターがないとエラーになるので
			if(ITPlanterClass.getSystemPlanterSize()>0){
			String result=sendCom.sendcom(" "+(ITPlanterClass.getCurrentPlanterNo()+1)+" -e N");
			String[] s=result.split(System.getProperty("line.separator"));
			if(s.length>1){
			String[] t=s[1].split(" ");
			if(t[3].equals("device")==false){
			pC2AutoTime.setText(t[3]);
			} else {
	...
	private JTextField getManu2AutoTime() {
		if (manu2AutoTime == null) {
			...
			if(ITPlanterClass.getSystemPlanterSize()>0){
			// あれ？　何で＋１になるの？
			String pNo=String.valueOf(ITPlanterClass.getCurrentPlanterNo()+1);
			
			String result=sendCom.sendcom(" "+pNo+" -e M");
			String[] s=result.split(System.getProperty("line.separator"));
			if(s.length>1){
			String[] t=s[1].split(" ");
			if(t[1].equals(":")==false){
				manu2AutoTime.setText(t[1]);
			} else {
...
//		System.out.println("CompData Saved");
	...
	if(ITPlanterClass.getSystemPlanterSize()>0){
	JTextField jta=(JTextField)arg0.getSource();
	message="PC->Auto復帰時間 "+jta.getText();
	String result=sendCom.sendcom(" "+(n+1)+" -e N"+jta.getText());
	itp_Logger.logger.info(result);
...
public void mouseClicked(MouseEvent arg0) {
	...
	if(((JTextComponent) arg0.getSource()).getName().equals("photoHolder")==true){
		...
		file.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
			...
			if(checkSaveHolder.isSelected()==true)
