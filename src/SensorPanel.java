...
public class SensorPanel extends JPanel implements ActionListener{
	...
	{	
        ...
        shortTimer.schedule(new TaskSensorShort(), 0, 7500); // タスクの実行間隔は5秒
        //else 			  shortTimer.schedule(new TaskSensorShort(), 0, 50000); // タスクの実行間隔は50秒
	}
	...
	public void setSensorLongTime()
	{
		...
		String[] sr2=sr.split(" ");
		if(sr2.length>1){
			String sr3=sr2[1];//.substring(0,sr2[1].length()-1);
...
System.out.println("SensorPanel: periodicTime = "+periodicTime);			
	...
	public void startSensorLongTimer()
	{
		...
		longTimer.schedule(new TaskSensorLong(), 60000/* delay after 1min */, periodicTime); // タスクの実行間隔は6時間(periodicTime)に１度
...
System.out.println("SensorPanelClass: currentCamName="+currentCamName);
				if(currentCamName.contains("-remove-")== true) continue;// skip removed camera
